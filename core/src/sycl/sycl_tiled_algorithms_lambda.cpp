...
)
{
    ...
    for (std::size_t k = 0; k < n_tiles; k++)
    {
        ...
        loss_tiled[k] = hpx::dataflow(
            ...
            { return compute_loss(training_input, training_output, sek_params, n_tile_size, sycl_device) },
    ...
    for (std::size_t c = 0; c < m_tiles; ++c)
    {
        for (std::size_t k = 0; k < m_tiles; ++k)
        {
            for (std::size_t m = 0; m < n_tiles; ++m)
            {
                ...
                // );
                ft_priorK[c * m_tiles + k] = hpx::dataflow(
                    [&queue, n_tile_size, m_tile_size, sycl_device](
                        hpx::shared_future<double *> f_A,
                        hpx::shared_future<double *> f_B,
                        hpx::shared_future<double *> f_C
                    )
...
)
{
    for (std::size_t i = 0; i < m_tiles; i++)
    {
        ...
        ft_vector[i] = hpx::dataflow(
            ...
            { return diag_posterior(A, B, m_tile_size, sycl_device); },
...
)
{
    for (std::size_t i = 0; i < m_tiles; i++)
    {
        ...
        ft_vector[i] = hpx::dataflow(
            [m_tile_size, sycl_device](const hpx::shared_future<double *> A)
            { return diag_tile(A, m_tile_size, sycl_device); },
