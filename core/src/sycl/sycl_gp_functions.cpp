...
)
{
    ...
    auto alpha_tiles = assemble_alpha_tiles(d_training_output, n_tiles, n_tile_size, sycl_device);
    ...
    auto prediction_tiles = assemble_tiles_with_zeros(m_tile_size, m_tiles, sycl_device);


    right_looking_cholesky_tiled(d_tiles, n_tile_size, n_tiles, sycl_device);

    // Triangular solve K_NxN * alpha = y
    forward_solve_tiled(d_tiles, alpha_tiles, n_tile_size, n_tiles, sycl_device);
    backward_solve_tiled(d_tiles, alpha_tiles, n_tile_size, n_tiles, sycl_device);
    ...
    gprat::sycl_backend::free_lower_tiled_matrix(d_tiles, n_tiles, sycl_device);
...
)
{
    ...
    auto d_alpha_tiles = 
        assemble_alpha_tiles(d_training_output, n_tiles, n_tile_size, sycl_device);

    auto d_prior_K_tiles = 
        assemble_prior_K_tiles(d_test_input, m_tiles, m_tile_size, n_regressors, sek_params, sycl_device);
    ...
    auto d_prior_inter_tiles = assemble_tiles_with_zeros(m_tile_size, m_tiles, sycl_device);

    auto d_prediction_tiles = assemble_tiles_with_zeros(m_tile_size, m_tiles, sycl_device);

    // Assemble placeholder for uncertainty
    auto d_prediction_uncertainty_tiles = assemble_tiles_with_zeros(m_tile_size, m_tiles, sycl_device);

    right_looking_cholesky_tiled(d_K_tiles, n_tile_size, n_tiles, sycl_device);

    // Triangular solve K_NxN * alpha = y
    forward_solve_tiled(d_K_tiles, d_alpha_tiles, n_tile_size, n_tiles, sycl_device);
    backward_solve_tiled(d_K_tiles, d_alpha_tiles, n_tile_size, n_tiles, sycl_device);
    ...
    symmetric_matrix_matrix_diagonal_tiled(
        d_t_cross_covariance_tiles, d_prior_inter_tiles, n_tile_size, m_tile_size, n_tiles, m_tiles, sycl_device);

    // Compute predicition uncertainty
    vector_difference_tiled(
        d_prior_K_tiles, d_prior_inter_tiles, d_prediction_uncertainty_tiles, m_tile_size, m_tiles, sycl_device);

    // Get predictions and uncertainty to return them
    std::vector<double> prediction = 
        copy_tiled_vector_to_host_vector(d_prediction_tiles, m_tile_size, m_tiles, sycl_device);

    std::vector<double> pred_var_full =
        copy_tiled_vector_to_host_vector(d_prediction_uncertainty_tiles, m_tile_size, m_tiles, sycl_device);
    ...
    gprat::sycl_backend::free_lower_tiled_matrix(d_K_tiles, n_tiles, sycl_device);
...
)
{
    ...
    auto d_alpha_tiles = assemble_alpha_tiles(d_training_output, n_tiles, n_tile_size, sycl_device);

    auto d_prior_K_tiles =
        assemble_prior_K_tiles_full(d_test_input, m_tiles, m_tile_size, n_regressors, sek_params, sycl_device);
    ...
    symmetric_matrix_matrix_tiled(
        d_t_cross_covariance_tiles, d_prior_K_tiles, n_tile_size, m_tile_size, n_tiles, m_tiles, sycl_device);

    // Compute predicition uncertainty
    matrix_diagonal_tiled(d_prior_K_tiles, d_prediction_uncertainty_tiles, m_tile_size, m_tiles, sycl_device);
    ...
    gprat::sycl_backend::free_lower_tiled_matrix(d_prior_K_tiles, m_tiles, sycl_device);
    gprat::sycl_backend::free(d_cross_covariance_tiles, queue);
    gprat::sycl_backend::free(d_t_cross_covariance_tiles, queue);
    gprat::sycl_backend::free(d_prediction_tiles, queue);
    gprat::sycl_backend::free(d_prediction_uncertainty_tiles, queue);
...
)
{
    ...
    auto d_y_tiles = assemble_y_tiles(d_training_output, n_tiles, n_tile_size, sycl_device);
    ...
    hpx::shared_future<double> loss_value =
        compute_loss_tiled(d_K_tiles, d_alpha_tiles, d_y_tiles, n_tile_size, n_tiles, sycl_device);
